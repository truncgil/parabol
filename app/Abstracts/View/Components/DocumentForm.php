...
abstract class DocumentForm extends Base
{
    ...
    protected function getContacts($type, $contacts)
    {
        ...
        if ($contact_type) {
            $contacts = Contact::$contact_type()->enabled()->orderBy('updated_at','desc')->take(setting('default.select_limit'))->get();
        } else {
            $contacts = Contact::enabled()->orderBy('updated_at','desc')->take(setting('default.select_limit'))->get();
        }
