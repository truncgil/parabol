        ...
        if ($this->canAccessMenuItem($title, ['read-sales-invoices', 'read-sales-revenues', 'read-sales-customers'])) {
            $menu->dropdown($title, function ($sub) use ($attr) {
                ...
                if ($this->canAccessMenuItem($title, 'read-sales-invoices')) {
                    ...
                    $sub->route('invoices.index', "Taslak Faturaları", ['search'=>'status:draft'], 10, $attr);
                    $sub->route('invoices.index', "Ödenen Faturalar", ['search'=>'status:paid'], 10, $attr);
                    $sub->route('invoices.index', "Kısmi Ödenen Faturalar", ['search'=>'status:partial'], 10, $attr);
                    $sub->route('invoices.index', "İptal Edilen Faturalar", ['search'=>'status:cancelled'], 10, $attr);
                    $sub->route('invoices.index', "Gönderilen Faturalar", ['search'=>'status:sent'], 10, $attr);
                }
        ...
        if ($this->canAccessMenuItem($title, ['read-purchases-bills', 'read-purchases-payments', 'read-purchases-vendors'])) {
            $menu->dropdown($title, function ($sub) use ($attr) {
                ...
                if ($this->canAccessMenuItem($title, 'read-purchases-bills')) {
                    ...
                    $sub->route('bills.index', "Taslak Faturaları", ['search'=>'status:draft'], 10, $attr);
                    $sub->route('bills.index', "Ödenen Faturalar", ['search'=>'status:paid'], 10, $attr);
                    $sub->route('bills.index', "Kısmi Ödenen Faturalar", ['search'=>'status:partial'], 10, $attr);
                    $sub->route('bills.index', "İptal Edilen Faturalar", ['search'=>'status:cancelled'], 10, $attr);
                    $sub->route('bills.index', "Alınan Faturalar", ['search'=>'status:received'], 10, $attr);
                }
